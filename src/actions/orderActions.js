...
const createOrder = (order) => async (dispatch, getState) => {
  try {
    ...
    const { data: { data: newOrder } } = await Axios.post(url_base+"/api/orders", order, {
      headers: {
...
const listMyOrders = () => async (dispatch, getState) => {
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.get(url_base+"/api/orders/mine", {
      headers:
        { Authorization: 'Bearer ' + userInfo.token }
    });
...
const listOrders = () => async (dispatch, getState) => {
  ...
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.get(url_base+"/api/orders", {
      headers:
        { Authorization: 'Bearer ' + userInfo.token }
    });
...
const detailsOrder = (orderId) => async (dispatch, getState) => {
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.get(url_base+"/api/orders/" + orderId, {
      headers:
        { Authorization: 'Bearer ' + userInfo.token }
    });
...
const payOrder = (order, paymentResult) => async (dispatch, getState) => {
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.put(url_base+"/api/orders/" + order._id + "/pay", paymentResult, {
      headers:
        { Authorization: 'Bearer ' + userInfo.token }
    });
